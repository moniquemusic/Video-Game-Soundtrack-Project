...
#scores were collected from metacritic.com, a site that aggregates scores from a multitude of gaming websites and also receives user imput directly
#sales estimates were collected from vgchartz.com
#and the soundtrack data was collected from Youtube using their API
#video game soundtracks are often uploaded to youtube by fans, and this is often the only streamable source due to game soundtracks not being covered by, and dare I say not taken seriously by, other streaming platforms

#there may be times with R stalls out and is unable to fully complete a for-loop
#at that time there are instructions on how to figure out which was the last loop completed based on what was last printed
#then the counter the for-loop goes through should be reduced to start at the next number

#but human interaction is absolutely required at three locations
#they are currently at lines 952, 976, 1584
#another way to find these lines is to search: THIS NEXT LINE REQUIRES HUMAN INPUT
...
#create the basic metacritic url to get basic game information from
#"%s" is a placeholder for where the year needs to be inputted
the_page<- "http://www.metacritic.com/browse/games/score/metascore/year/pc/filtered?year_selected=%s&sort=desc"

#custom function to create url for scraping
#a year is entered for "x"
create_scrape_url <- function(x) {paste0(sprintf(the_page, x))}

#create a list of urls to run through JSON extraction function
metcrit_url_list<- sapply(years, create_scrape_url)

#create blank lists to store data generated from for-loop
all_game_titles<- c()
all_game_dates<- c()
all_game_critic_scores<- c()
all_game_user_ratings<- c()

#for-loop collects data from the top 100 games from each year (name, release date, critic score, and user score)
for(metcrit in metcrit_url_list){
  ...
  all_game_critic_scores <- append(all_game_critic_scores, game_scores)
...
game_title_df <-data.frame(all_game_titles, all_game_dates, all_game_critic_scores, all_game_user_ratings)

#remove the white space in front of game titles
#custom function to remove white space
trim <- function (x) gsub("^\\s+|\\s+$", "", x)

#update game_title_df with game titles sans blank spaces
...
#SEARCH_API
#use the youtube search API to get the return results of each game title being searched along with "soundtrack"
...
#create a list of game titles from 2016 to be the elements inserted into the search API
...
#run the urls through the get_JSON function to get the JSON data from the youtube search API
...
#search_JSON has 1000 entries due to there being 100 games tested per year for 10 years

#create a counter from the number of rows in game_title_df to run through the next for-loop
rows<-c(1:nrow(game_title_df))

#store the length of the result each game returns
row_length_list<- c()

#for each game add the length of the API result to the list
for(row in rows){
...
#create a empty list to store bad entries this year
bad_line <-c()

#filter out the rows in search_JSON that are the wrong length
for(row in rows){
...
#convert the list of bad rows, to the list of good rows
row2<- rows[-bad_line]

#if the dataframe within each row of search_JSON does not have the right number of rows, it also has to be taken out
#the rows with bad dataframes are added to the list of bad rows
for(row in row2){
...
#remove inconsistent lines in search_JSON, and turn it into a dataframe
search_JSON_df <- data.frame(search_JSON[-bad_line])

#remove inconsistent lines in search_JSON from game_title_df
game_title_df_2<- game_title_df[-bad_line,]
...
#custom function to specify the "title" from ".item.snippet"
...
title_number <- c(1:50)
#due to the load this next for-loop puts on R, the segment length will be limited
...
#if R stalls out:
#in the next line, removed the # and replace HERE with last game title printed
#grep(HERE, colnames(titles2_df))
#in the next line, removed the # and replace HERE with the result from the line above
#gg_games<- c(HERE:ncol(titles2_df))
#then run the for-loop again
#repeat this until the for loop goes through the last gg_games
#reset gg_games back to c(1:ncol(titles2_df))



#update game_title_df to match what is in titles2_df
#remove lines no longer appear in the columns of titles_df
bad_line_4<- c()

num_game_title_df<- c(1: nrow(game_title_df))

for(num in num_game_title_df){
  removal<- sort(stringdist(game_title_df$all_game_titles[num], colnames(titles_df)))[1]
  removal<- paste0(" ", removal, " ")
  if(removal != " 0 "){
    bad_line_4<- append(bad_line_4, num)
  }
}

#create the next version of game_title_df
game_title_df_2<-game_title_df[-bad_line_4,]

#store the new dataframe
write_csv(game_title_df_2, "game_title_df_2.csv")
...
#custom function to create url for the stats API
#url requires etag of video (not the title like the previous API)
...
#custom function to further reduce dataframe and apply to above dataframe
...
soundtrack_data_df_2<-data.frame()
...
for(gg_g in gg_games){
  ...
  if(is.na(table(grepl("TRUE", segment_valid_collection))["TRUE"]) == TRUE){
    ...
    for(lengthh in segment_lengthh){
      ...
      for(startt in segment_startt){
        temp_title<- gsub("\\.", " ", colnames(titles2_df[gg_g]))
        segment<-substr(temp_title, start=startt, stop=(startt + lengthh2))
  ...
  if(length(relevent_titles) > 0){
    ...
    if(length(keep_rows) > 0){
      ...
      soundtrack_data_df_2<-rbind(soundtrack_data_df_2, holder_data_frame) 
      ...
      bad_line_2<- append(bad_line_2, gg2)
      
      #add NA to num_of_useable_videos
      num_of_useable_videos<- append(num_of_useable_videos, NA)
  ...
  print(colnames(titles2_df[gg_g]))
}

write_csv(soundtrack_data_df_2, "used_videos_df_3.csv")
...
#look for games that don't appear in soundtrack_data_df_2
for(gg_g in gg_games){
  removal<- sort(stringdist(colnames(titles2_df[gg_g]), soundtrack_data_df_2$game_name))[1]
  removal<- paste0(" ", removal, " ")
  if(removal != " 0 "){
    bad_line_3<- append(bad_line_3, gg_g)
  }
}

#remove un-useable rows from game_title_df_2
if(length(bad_line_2) > 0){
  game_title_df_2<- game_title_df_2[-bad_line_2,]
}
...
final_game_title_df <- data.frame(game_title_df_2[-bad_line_3,])
...
#removed unecessary elements from num_of_useable_videos
...
#store stats into their respective lists
video_num_list<- c()
total_views_list<- c()
total_likes_list<- c()
total_dislikes_list<- c()
total_favorites_list<- c()
total_comments_list<- c()
...
for(gg_g in gg_games_2){
  ...
  video_num<- num_of_useable_videos[gg_g]
  ...
  video_num_list<- append(video_num_list, video_num)
  total_views_list<- append(total_views_list, total_views)
  total_likes_list<- append(total_likes_list, total_likes)
  total_dislikes_list<- append(total_dislikes_list, total_dislikes)
  total_favorites_list<- append(total_favorites_list, total_favorites)
  total_comments_list<- append(total_comments_list, total_comments)
  
  print(final_game_title_df$all_game_titles[gg_g])
}

#create dataframe with all the stats
game_stats_df<- cbind(video_num_list, total_views_list, total_likes_list, total_dislikes_list, total_favorites_list, total_comments_list)
...
write_csv(final_game_title_df, "final_game_title_df_updated_5.csv")

View(final_game_title_df)

#convert the all_game_titles column in final_game_title_df to type character
final_game_title_df$all_game_titles<- as.character(final_game_title_df$all_game_titles)
...
#more information was gotten from metacritic
#the first clump of information came from the metacritic page that lists the names, release dates, and scores of the top 100 games from a specific year
#example link: http://www.metacritic.com/browse/games/score/metascore/year/pc/filtered?year_selected=2016&sort=desc
#links from that page were scraped using Chrome extension "Link Klipper" (https://chrome.google.com/webstore/detail/link-klipper-extract-all/fahollcgofmpnehocdgofnhkkchiekoo?hl=en)
#100 links were taken per original metacritic page
#those links lead to a page dedicated for each game, which has more information about the games
#example link: http://www.metacritic.com/game/pc/out-of-the-park-baseball-17

#set working directory to the file with the links, please replace my directory with yours
#THIS NEXT LINE REQUIRES HUMAN INPUT
...
#create a counter for the list of links scraped
num_of_links<- c(1:length(meta_link_files))

#create an empty dataframe to store the links from the metacritic csv files
links_df<- data.frame()

#for-loop opens csv into R, then combines them all together in one dataframe
for(link in num_of_links){
  temp_file<- read.csv(meta_link_files[link])
  
  links_df<- rbind(links_df, temp_file)
}

#each csv file had a column with the link, and a column (Anchor.Text) with the words the hyperlink was attached to
#convert the Anchor.text column to character
links_df$Anchor.Text<- as.character(links_df$Anchor.Text)

#set working directory to original directory, please replace my directory with yours
#THIS NEXT LINE REQUIRES HUMAN INPUT
...
#create a counter for how many games are from final_game_title_df (probably 1000)
num_of_final_game_title <- c(1:nrow(final_game_title_df))

#create empty list to store links in correct order
list_of_links<-c()

#reorder links from links_df to match the order of games in final_game_title_df
#do this by comparing the game title in final_game_title_df$all_game_titles vs the game title in links_df$Anchor.Text
for(num in num_of_final_game_title){
  
  #get the stringdist comparing the game title from final_game_title_df to the game titles in links_df
  string_distances<- stringdist(final_game_title_df$all_game_titles[num], links_df$Anchor.Text)
  
  #sometimes the game name from each dataframes is not exactly the same
  #sometimes there are weird spaces at the begining or end of the game titles
  #therefore some leniency is allowed in stringdist
  #stringdist 0-2 are accepted as a good enough match
  if(sort(string_distances)[1] == 0){
    string2<- paste0(" ", string_distances, " ")
    spot<- grep(" 0 ", string2)
    list_of_links<- append(list_of_links, as.character(links_df$Link[spot]))
  }else if(sort(string_distances)[1] == 1){
    string2<- paste0(" ", string_distances, " ")
    spot<- grep(" 1 ", string2)
    list_of_links<- append(list_of_links, as.character(links_df$Link[spot]))
  }else if(sort(string_distances)[1] == 2){
    string2<- paste0(" ", string_distances, " ")
    spot<- grep(" 2 ", string2)
    list_of_links<- append(list_of_links, as.character(links_df$Link[spot]))
  }else{
...
#create a dataframe to contain game names and their metacritic link
#dataframe is named "game_type_df" because we hope to get the type aka the genre of the game 
game_type_df<- data.frame(game_title_df_2$all_game_titles, list_of_links)

#name the game_type_df columns
colnames(game_type_df) <- c("game_title", "meta_link")

#create a counter of games in game_type_df to run through for-loop
num_final_games<- c(1:nrow(game_type_df))


#game specific pages list the number of critics and the number of users that reviewed the game
#game specific pages also lists the genres the game is catagorized under, and the age-appropriate rating (PG vs M, etc)
...
for(num in num_final_games){
  ...
  if(is.na(game_type_df$meta_link[num])){
    ...
    #result return like this: "\n8.5\n    "  "3734 Ratings"
    #arrange data to end up with 3734 from the previous example
    user_num_2<- page_data %>% 
      html_nodes(".feature_userscore a") %>%
      html_text()
    
    if(table(grepl("Ratings", user_num_2))["TRUE"] > 0){
      
      user_num_3<- user_num_2[grep("Ratings", user_num_2)]
      user_num<- as.numeric(unlist(strsplit(x = user_num_2, split = " "))[1])
    }else{
      user_num<- NA
    }
    
    #get the type (genre) of the game, then convert to character
    game_type<- page_data %>% 
    ...
    game_type<- paste(game_type, collapse= ", ")
  ...
  if(length(all_critic_num) != length(all_user_num)){
    print("something's wront")
    print(num)
  }
}
...
write_csv(additional_data, "additional_data_df_2.csv")





#more information can be extracted from the game specific links
#critic and user reviews are catagorized as positive, mixed and negative
#this data can be collected

#create and store critic and user review links
#custom function to create the url to retrieve critic and user reviews
create_critic_url<- function(x){paste0(x, "/critic-reviews", collapse = "")}

#create an store all critic urls in one place
all_critic_urls<- sapply(game_type_df$meta_link, create_critic_url)

#positive, mixed, and negative scores are grabbed at once and need to be separated
#collect the three types of critic and user reviews and put into all_breakdown
all_breakdown<- c()

#for-loop scrapes each url for the reviews
for(num in num_final_games){
...
#separate all_breakdown into their respective review types
...
#store dataframe
write_csv(critic_user_reviews_df, "critic_user_reviews_df_2.csv")
...
#Video Game Sales
#collect sale estimates from http://www.vgchartz.com/
#the links were gotten from the webpage with the same technique as the metacritic page
#the page with top selling PC games http://www.vgchartz.com/platform/48/microsoft-windows/ was scraped for the games's link to their specific page
#the number of links forced the scraping to be done in two batches
#links were saved to vgchartz.csv and vgchartz_2.csv


#import csv of links from vgchartz page
vgchartz_testest<-read.csv("vgchartz.csv") 

#filter out rows that are missing Links or Anchor.Text
vgchartz_df<-vgchartz %>% 
  filter(nchar(as.character(Link)) > 0) %>% 
  filter(nchar(as.character(Anchor.Text)) > 0)
...
#links in vgchartz_raw_df are in the order they appear on vgchartz.com
#the links need to be re-ordered to match order in final_game_title_df

#create a counter for the next for-loop
num_final_games<- c(1: nrow(final_game_title_df))

#create lists to store for-loop results into
vgchartz_links<-c()
...
#game titles in final_game_title_df are usuaslly longer and more specific than the game titles from vgchartz_raw_df
#therefore a perfect match is not necessary
#stringdist of 0 or 1 are allowed
for(num in num_final_games){
  #create two counters
  n<- c(1)
  c<- c(1)
  
  #because game titles in final_game_title_df are usuaslly longer and more specific than the game titles from vgchartz_raw_df
  #a segment of the title from final_game_title_df will be used to compare to the titles in vgchartz_raw_df

  #control where the game segment ends
  #create a number range from the max number of characters in final_game_title_df to 5
  #this way, the segment will first be the full length of the title, then be shortened one character at a time until a match is found
  end_segment<- (nchar(final_game_title_df$game_title[num]): 5)
  
  #when a match is found n will increase to 2 and end the while-loop
  while(n < 2){
    ...
    #if there is an exact match between the final_game_title_df segment and vgchartz_raw_df
    if(sort(stringdist(game_title_subset, vgchartz_raw_df$Anchor.Text))[1] == 0){
      
      #to make sure grep finds "0" and not "30" or "104"
      #add a space before and after every stringdist value and search for " 0 "
      match_spot<- grep(" 0 ", paste0(" ", stringdist(game_title_subset, vgchartz_raw_df$Anchor.Text), " "))[1]
      
      #store the Anchor.Text from vgchartz_raw_df that the segment matched to
    ...
    if(sort(stringdist(game_title_subset, vgchartz_raw_df$Anchor.Text))[1] > 1){
      ...
      #this will shorten the segment of final_game_title_df being compared by one character
      c<- c+1
    }
    
    #each time game_title_subset lacks a match it will shrink until it has the character length of 5
    #eventually c will reach the maximum size, and there is no useable link for the game
...
#change the data in vgchartz_df to a more appropriate data type
...
#create an empty list to store the data from the for-loop into
vgchartz_global_sales<-c()

#for-loop scraped the global sale data from each link in vgchartz_df
for(num in num_vgchartz_df){
  ...
  if(is.na(vgchartz_df$links[num])){
    ...
    #add NA to the list of it doesn't
...
vgchartz_df$global_sales<- as.numeric(vgchartz_df$global_sales)




#some games from our final_game_title_df are not in vgchartz_raw_df
#therefore we have to search the title within the vgchartz webpage and scrape the results

#find where links in vgchartz_df are missing
NA_vgchartz<- which(is.na(vgchartz_df$global_sales))

#create a counter for the next for-loop
num_NA_vgchartz<- c(1: length(NA_vgchartz))

#create empty lists to store the data from the for-loop into
vgchartz_game_name_3<- c()
vgchartz_links_3<- c()
vgchartz_global_sales_3<- c()

#for-loop creates the necessary url, then scrapes the webpage
#searching for the title returns every console the game has been released on (PC, Xbox, PlayStation, etc)
#there is no difference in the code between a line about the PC version and all the other versions
#therefore all the results must collected and then be de-tangled to pick out just the PC sales
for(num in num_NA_vgchartz){
  ...
  #create the url with the search term (the game title)
  ...
  if(length(game_chart) == 0){
    ...
    #the value right before "PC" was always the title
    #the 8th value after "PC" was always the global sale
    if(length(PC_spots) > 1){
      num_PC_results<- c(1: length(PC_spots))
      
      #if more than one PC game is in the search results, store all of the values
...
#sometime the for-loop above times out, so do the following lines
#a) grep("THE LAST GAME TITLE PRINTED", vgchartz_df$game_title)
#b) grep("a result", NA_vgchartz)
#c) grep("c result", 8)
...
#match games titles from vgchartz_df_2 to vgchartz_df in order to fill in empty links in vgchartz_df

#reset the counter
num_NA_vgchartz <- c(1:length(NA_vgchartz))

#fill in the blank rows in vgchartz_df with the search data stored in vgchartz_df_2
for(num in num_NA_vgchartz){
...
#there are still empty spots in vgchartz_df
#give the games that sales cannot be found one more chance
still_NA_spots<- which(is.na(vgchartz_df$global_sales))

#store the titles of games with no sales data
...
#for-loop compares titles with the missing sales data from vgchartz_df and compares them to the titles vgchartz_df_2 with stringdist
for(num in num_still_NA_games){
  
  #find what the shortest stringdist result is
  best_match <- sort(stringdist(still_NA_games[num], vgchartz_df_2$game_name))[1]
  
  #find where the shortest stringdist result takes places
  best_match_spot<- grep(best_match, stringdist(still_NA_games[num], vgchartz_df_2$game_name))
  
  #sometimes the best match for a title is a large stringdist
  #this shows that the title doesn't really match anything
  #that's why this is limited to titles that only match well with 5 or less other titles
...
#write in numbers that were printed from the for-loop before
#THIS NEXT LINE REQUIRES HUMAN INPUT
useable_games<- c(14, 277, 338, 415, 431, 487)

#store which of the resulting game titles should be used
#if the first title printed under "game name contenders" seems to be the best fit, put 1
#if the second title printed under "game name contenders" seems to be the best fit, put 2
useable_title_spots<-c(1, 1, 1, 1)

#counter for next for-loop
num_useable_games<-c(1: length(useable_games))

#insert the global sales of the useable games from vgchartz_df_2 into vgchartz_df
